...
# itdb 二进制部署在线更新脚本：分页抓取 GitHub 全部 Release，选择版本后自动完成下载校验、停止服务、替换产物并重启

REPO="zyx3721/itdb-new"
INSTALL_DIR="${INSTALL_DIR:-/data/itdb}"
...
cleanup() {
    case "$TMP_DIR" in
        /tmp/itdb-new-update.*) [ -d "$TMP_DIR" ] && rm -rf "$TMP_DIR" ;;
    esac
}
trap cleanup EXIT
...
# get_current_version 通过 itdb -v 读取当前已部署的版本号
get_current_version() {
    if [ -x "$INSTALL_DIR/itdb" ]; then
        CURRENT_VERSION=$("$INSTALL_DIR/itdb" -v 2>/dev/null | head -n1 | awk '{print $2}')
    fi
}
...
show_version_list() {
    ...
    if [ -n "$CURRENT_VERSION" ]; then
        echo_log_info "当前系统 itdb 版本为 \033[32m$CURRENT_VERSION\033[0m ，仓库 $REPO 可安装的版本有（共 $total 个）：\n"
    else
        echo_log_warn "未检测到已安装的 itdb 版本，仓库 $REPO 可安装的版本有（共 $total 个）：\n"
    fi
...
confirm_install() {
    ...
    while true; do
        read -rp "即将${action} itdb ${TARGET_TAG}${tip}，是否继续? [y/N]: " yn
        case "$yn" in
            [Yy]*) break ;;
            [Nn]*|"")
...
prepare_assets() {
    ...
    BACKEND_ASSET="itdb_${ver}_linux_${ARCH}.tar.gz"
    FRONTEND_ASSET="itdb-frontend_${ver}.tar.gz"
...
download_packages() {
    echo ""
    echo_log_info "\033[32m开始下载 itdb ${TARGET_TAG} 安装包...\033[0m"
...
extract_packages() {
    ...
    [ -f "$TMP_DIR/backend/linux_${ARCH}/itdb" ] || echo_log_error "后端安装包内未找到 itdb 二进制文件"
...
install_packages() {
    ...
    if [ -f "$INSTALL_DIR/itdb" ]; then
        cp -f "$INSTALL_DIR/itdb" "$INSTALL_DIR/itdb.bak" || echo_log_error "备份后端二进制失败"
        chmod --reference="$INSTALL_DIR/itdb.bak" "$TMP_DIR/backend/linux_${ARCH}/itdb" 2>/dev/null
        chown --reference="$INSTALL_DIR/itdb.bak" "$TMP_DIR/backend/linux_${ARCH}/itdb" 2>/dev/null
    fi
    chmod +x "$TMP_DIR/backend/linux_${ARCH}/itdb"
    cp -f "$TMP_DIR/backend/linux_${ARCH}/itdb" "$INSTALL_DIR/itdb.new" || echo_log_error "写入新后端二进制失败"
    mv -f "$INSTALL_DIR/itdb.new" "$INSTALL_DIR/itdb" || echo_log_error "替换后端二进制失败"
...
rollback() {
    ...
    if [ -f "$INSTALL_DIR/itdb.bak" ]; then
        mv -f "$INSTALL_DIR/itdb.bak" "$INSTALL_DIR/itdb" && chmod +x "$INSTALL_DIR/itdb"
    fi
...
start_services() {
    ...
    [ $ok -eq 0 ] && rollback
    rm -f "$INSTALL_DIR/itdb.bak"
    rm -rf "$INSTALL_DIR/dist.bak"
    local new_version
    new_version=$("$INSTALL_DIR/itdb" -v 2>/dev/null | head -n1 | awk '{print $2}')
...
main() {
    ...
    detect_arch
    get_current_version
    TMP_DIR=$(mktemp -d /tmp/itdb-new-update.XXXXXX)
    fetch_all_releases
    show_version_list
    select_version
    confirm_install
