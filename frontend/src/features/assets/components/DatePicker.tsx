...
export function DatePicker({
  ...
  function toggleCalendar() {
    ...
    if (rect) setPosition(calendarPosition(rect, dialog, panelMinWidth));
    setMonthPickerOpen(false);
    setOpen(true);
  }
  ...
  return (
    <div className="relative">
      <Input
        ...
        aria-label="到期日期"
      />
      <div className="absolute right-1 top-1/2 inline-flex -translate-y-1/2">
        <button
          type="button"
          onClick={toggleCalendar}
          className="itdb-action-button grid h-8 w-8 place-items-center rounded-md text-[var(--itdb-text-muted)]"
          aria-label="选择到期日期"
        >
...
function calendarPosition(rect: DOMRect, dialog: HTMLElement | null, panelMinWidth: number) {
  const width = Math.max(rect.width, panelMinWidth);
  const gutter = 12;
  const panelHeight = 360;
  const gap = 8;
  const showAbove = (bottomBoundary: number, topBoundary: number) =>
    bottomBoundary - rect.bottom - gap < panelHeight &&
    rect.top - topBoundary > bottomBoundary - rect.bottom;
  if (dialog) {
    const dialogRect = dialog.getBoundingClientRect();
    const above = showAbove(dialogRect.bottom, dialogRect.top);
    const maxTop = dialogRect.height - panelHeight - gutter;
    return {
      left: Math.max(
        gutter,
        Math.min(rect.left - dialogRect.left, dialogRect.width - width - gutter)
      ),
      width,
      top: above
        ? rect.top - dialogRect.top - panelHeight - gap
        : Math.min(rect.bottom - dialogRect.top + gap, Math.max(gutter, maxTop)),
      fixed: false,
    };
  }
  ...
  return {
    left,
    width,
    top: showAbove(window.innerHeight, 0)
