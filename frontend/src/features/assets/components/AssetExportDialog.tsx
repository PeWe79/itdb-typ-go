...
export function AssetExportDialog({
  ...
  useEffect(() => {
    if (!open) return;
    setFormat('xlsx');
    setFilename(`${resource.title}-${localTimestamp()}`);
  ...
  const previewName = `${sanitizeExportFileName(
    filename || `${resource.title}-${localTimestamp()}`
  )}.${format}`;
  ...
  function handleExport() {
    ...
    void trackAuditEvent({ type: `export:${resource.key}`, count: rows.length });
    toast.success(`已导出 ${rows.length} 条${resource.title}数据`);
    onOpenChange(false);
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="border-b border-[var(--itdb-border)] px-5 py-4">
          <DialogTitle>导出{resource.title}</DialogTitle>
          <DialogDescription>
            将当前筛选后的 {rows.length} 条{resource.title}数据进行导出
          </DialogDescription>
        </DialogHeader>
