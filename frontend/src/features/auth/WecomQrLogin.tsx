...
export function WecomQrLogin({ onSuccess }: WecomQrLoginProps) {
      ...
      try {
        ...
        instance = ww.createWWLoginPanel({
          ...
          params: {
            ...
            color_scheme:
              document.documentElement.dataset.itdbTheme === 'light'
        ...
        if (instance.el) {
          ...
          instance.el.style.border = '1px solid var(--itdb-border)';
        }
      } catch {
        instance = null;
      }
  ...
  if (loadError) {
    return (
      <div
        ...
        style={{ background: 'var(--itdb-control-bg)', border: '1px solid var(--itdb-border)' }}
      >
  ...
  if (!embed) {
    return (
      <div
        ...
        style={{ background: 'var(--itdb-control-bg)', border: '1px solid var(--itdb-border)' }}
      >
        <Loader2 size={24} className="itdb-spinner" style={{ color: 'var(--itdb-accent-text)' }} />
      </div>
    );
  }
  ...
  return (
    <iframe
      ...
      style={{ height: 420, border: '1px solid var(--itdb-border)', background: '#ffffff' }}
    />
  );
}
