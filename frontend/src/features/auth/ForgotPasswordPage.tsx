  ...
  return (
    ...
    >
      ...
      <section className="relative z-10 flex w-full max-w-[460px] flex-col items-center gap-5">
        <Brand brand={brand} />
        <section className="itdb-login-frame itdb-login-reveal w-full rounded-[28px] p-1">
          <div className="rounded-[24px] border border-[var(--itdb-border)] bg-[var(--itdb-login-panel-bg)] p-6 shadow-[var(--itdb-login-panel-shadow)] backdrop-blur sm:p-8">
            <div className="mb-6 text-center">
              <h1 className="text-2xl font-bold">忘记密码</h1>
              <p className="mt-2 text-sm text-[var(--itdb-text-muted)]">
                {step === 'identity'
                  ? '请输入您需要找回密码的用户名'
            ...
            {step === 'identity' ? (
              <form className="space-y-5" onSubmit={verifyIdentity} noValidate>
                <AuthField
                  label="用户名"
                  required
                  icon={<User size={17} />}
                  action={
                    <Link
                      to="/login"
                      className="inline-flex items-center gap-1 text-xs font-semibold text-[var(--itdb-accent-text)]"
                    >
                      <ArrowLeft size={14} />
                      返回登录
                    </Link>
                ...
                <AuthField label="验证码" required icon={<KeyRound size={17} />}>
                  <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_132px]">
                    ...
                    <button
                      ...
                      className="itdb-action-button flex h-12 items-center justify-center gap-2 rounded-2xl border px-4 text-sm font-semibold"
              ...
              <form className="space-y-5" onSubmit={confirm} noValidate>
                <AuthField
                  label="验证邮箱"
                  required
                  icon={<Mail size={17} />}
                  action={
                    <span className="text-xs font-semibold text-[var(--itdb-text-muted)]">
                      当前账号：<span className="text-[var(--itdb-text)]">{username}</span>
                    </span>
                  }
                >
                  <input
                ...
                <AuthField label="重置验证码" required icon={<KeyRound size={17} />}>
                  <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_96px]">
                    <input
                      ...
                      className="itdb-auth-input min-w-0 flex-1"
                    ...
                    <button
                      ...
                      onClick={() => void resend()}
                      className="itdb-login-submit flex h-12 items-center justify-center gap-2 rounded-2xl text-sm font-semibold text-white disabled:opacity-60"
                    >
                ...
                <AuthField label="新密码" required icon={<Lock size={17} />}>
                  <span className="relative block">
                    <input
                      ...
                      className="itdb-auth-input pr-12"
...
function Brand({ brand }: { brand: ReturnType<typeof useBrandSettings> }) {
  return (
    <div className="itdb-login-reveal flex flex-col items-center text-center">
      ...
      <p className="mt-1 text-xs uppercase tracking-[0.28em] text-[var(--itdb-text-muted)]">
        {brand.appSubtitle}
      </p>
    </div>
  );
...
function AuthField({
  ...
  return (
    <label className="block">
      ...
      <span className="relative block">
        <span className="absolute left-4 top-6 z-10 -translate-y-1/2 text-[var(--itdb-text-muted)]">
          {icon}
        </span>
        <span className="block [&_.itdb-auth-input]:h-12 [&_.itdb-auth-input]:w-full [&_.itdb-auth-input]:rounded-2xl [&_.itdb-auth-input]:border [&_.itdb-auth-input]:border-[var(--itdb-border)] [&_.itdb-auth-input]:bg-[var(--itdb-control-bg)] [&_.itdb-auth-input]:py-3 [&_.itdb-auth-input]:pl-12 [&_.itdb-auth-input]:text-sm [&_.itdb-auth-input]:text-[var(--itdb-text)] [&_.itdb-auth-input]:outline-none">
          {children}
        </span>
      </span>
    </label>
...
function SubmitButton({ busy, label }: { busy: boolean; label: string }) {
  return (
    <button
      type="submit"
      disabled={busy}
      className="itdb-login-submit flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[linear-gradient(135deg,#2563eb,#06b6d4)] text-sm font-semibold text-white shadow-[0_18px_48px_rgba(37,99,235,0.35)] disabled:opacity-60"
    >
