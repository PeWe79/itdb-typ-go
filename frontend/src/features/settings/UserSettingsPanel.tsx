...
export function UserSettingsPanel({ canManage }: { canManage: boolean }) {
  ...
  async function deleteUser(user: SettingsUser) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : '删除用户失败');
    }
  }
  ...
  async function deleteGroup(group: UserGroup) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : '删除用户群组失败');
    }
  }
  ...
  async function deleteUserRole(role: Role) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : '删除用户角色失败');
    }
  }
  ...
  async function toggleUserDisabled(user: SettingsUser) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : '更新用户状态失败');
    }
  }
  ...
  async function toggleGroupDisabled(group: UserGroup) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : '更新用户群组状态失败');
    }
  }
  ...
  async function toggleRoleDisabled(role: Role) {
    try {
      ...
      showErrorToast(err instanceof Error ? err.message : '更新用户角色状态失败');
    }
  }
  ...
  return (
    <>
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 overflow-hidden xl:grid-cols-[430px_minmax(0,1fr)]">
        <nav
          className="itdb-hidden-scrollbar max-h-64 space-y-2 overflow-y-auto rounded-lg border border-[var(--itdb-border)] bg-white/[0.035] p-2 xl:max-h-none"
          aria-label="用户配置"
        >
          {cards.map(card => {
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs text-[var(--itdb-text-muted)]">{itemCount}</span>
                  </span>
                  <span className="mt-1 block line-clamp-2 text-xs leading-5 text-[var(--itdb-text-muted)]">
                    {card.description}
                  </span>
                </span>
              </button>
        ...
        <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-[var(--itdb-border)] bg-white/[0.035] p-4">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-xs text-[var(--itdb-text-muted)]">
          ...
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-[var(--itdb-border)] bg-white/[0.02]">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--itdb-border)] p-3">
              <div className="flex items-baseline gap-2">
                <span className="text-sm font-semibold">{selectedCard.title}</span>
                <span className="text-xs text-[var(--itdb-text-muted)]">{count} 项</span>
              </div>
              <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
                ...
                {canManage ? (
                  <button
                    type="button"
                    className="itdb-action-button flex h-9 shrink-0 items-center gap-2 rounded-lg border px-3 text-sm"
                    onClick={() =>
                      active === 'users'
                        ? setUserEditor(null)
                        : active === 'groups'
            ...
            <div className="itdb-hidden-scrollbar min-h-0 flex-1 space-y-2 overflow-y-auto p-3">
              {loading ? (
              ...
              {showRows && active === 'groups'
                ? filteredGroups.map(group => (
                    <CompactRow
                      key={group.id}
                      title={group.name}
                      meta={`${(group.members ?? []).length} 个用户 · ${(group.roles ?? []).map(role => role.name).join('、') || '无角色'}`}
...
function CompactRow({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="itdb-action-button group grid min-h-[74px] w-full grid-cols-[3px_minmax(0,1fr)_auto] items-center gap-3 rounded-lg border p-0 pr-3 text-left"
      style={{
        background: active ? 'rgba(59,130,246,0.11)' : 'rgba(255,255,255,0.026)',
        borderColor: active ? 'rgba(96,165,250,0.52)' : 'var(--itdb-border)',
        color: 'var(--itdb-text)',
      }}
    >
      <span
      ...
      <span className="min-w-0 py-2">
        ...
        <span className="mt-1 block truncate text-xs" style={{ color: 'var(--itdb-text-muted)' }}>
          {meta}
        </span>
        <span className="mt-0.5 block truncate text-xs" style={{ color: 'var(--itdb-text-muted)' }}>
          {extra}
        </span>
      </span>
      {typeof enabled === 'boolean' ? (
        enabled ? (
          ...
          <ToggleLeft size={18} style={{ color: 'var(--itdb-text-muted)' }} />
        )
      ) : (
        <span
          className="h-2 w-2 rounded-full"
          style={{ background: active ? '#60a5fa' : 'var(--itdb-border)' }}
        />
      )}
    </button>
  );
