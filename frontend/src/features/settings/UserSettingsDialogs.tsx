...
export function UserEditorDialog({
  ...
  useEffect(() => {
    ...
    setPassword('');
    setRoleKeys(
      (user?.directRoles ?? []).length
        ? (user?.directRoles ?? []).map(role => role.key)
        : [user?.role || 'viewer']
    );
...
export function GroupEditorDialog({
  ...
  useEffect(() => {
    ...
    setRoleKeys(
      (group?.roles ?? []).length ? (group?.roles ?? []).map(role => role.key) : ['viewer']
    );
  }, [group, open]);
  async function save() {
...
export function EditorFrame({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className="itdb-dialog-panel flex max-h-[88vh] w-[min(92vw,720px)] flex-col gap-0 overflow-hidden p-0 sm:max-w-none"
        onOpenAutoFocus={onOpenAutoFocus}
      >
        <DialogHeader className="space-y-0 border-b border-[var(--itdb-border)] p-5 pr-14 text-left">
        ...
        <div className="itdb-hidden-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto p-5">
          {children}
        </div>
        {footer ? (
          <DialogFooter className="border-t border-[var(--itdb-border)] bg-white/[0.018] p-4 sm:justify-end">
            {footer}
          </DialogFooter>
        ) : null}
      </DialogContent>
...
export function SectionTitle({ title }: { title: string }) {
  return <div className="pt-1 text-sm font-semibold text-[var(--itdb-text)]">{title}</div>;
}
...
export function InlineRow({
  ...
  return (
    <label
      className={`grid grid-cols-1 gap-1.5 text-xs text-[var(--itdb-text-muted)] sm:grid-cols-[64px_minmax(0,1fr)] sm:gap-3 ${alignStart ? 'sm:items-start' : 'sm:items-center'}`}
    >
    ...
    <span className="relative block">
      ...
      <AppTooltip label={visible ? '隐藏密码' : '显示密码'} placement="top">
        <button
          ...
          className="itdb-action-button absolute right-2 top-1/2 grid h-7 w-7 -translate-y-1/2 place-items-center rounded-md disabled:cursor-not-allowed disabled:opacity-50"
  ...
  return (
    <section>
      <div className="mb-2 flex items-center justify-between gap-3">
        <h4 className="text-sm font-semibold text-[var(--itdb-text)]">{title}</h4>
        {!hideActions ? (
          <div className="flex gap-1">
            <button
              type="button"
              disabled={disabled || itemKeys.length === 0 || allSelected}
              className="itdb-action-button cursor-pointer rounded-md border px-2.5 py-1 text-[11px] transition disabled:cursor-not-allowed disabled:opacity-50"
              style={{
            ...
            <button
              type="button"
              disabled={
                disabled || itemKeys.length === 0 || !itemKeys.some(key => value.includes(key))
              }
              className="itdb-action-button cursor-pointer rounded-md border px-2.5 py-1 text-[11px] transition disabled:cursor-not-allowed disabled:opacity-50"
              style={{
      ...
      <div className="itdb-hidden-scrollbar grid gap-1.5 pb-1 pr-1">
        {items.map(item => {
          ...
          return (
            <label
              ...
              style={{
                ...
                boxShadow: `inset 0 0 0 1px ${active ? 'rgba(96,165,250,0.45)' : 'var(--itdb-border)'}`,
              }}
            >
              <input
              ...
              <span className="min-w-0">
                <span className="block truncate text-sm font-medium text-[var(--itdb-text)]">
                  {item.label}
                </span>
                <span className="mt-0.5 block text-xs leading-4 text-[var(--itdb-text-muted)]">
                  {item.helper}
                </span>
              </span>
