...
export function SettingsSplitLayout({
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-1 items-stretch gap-4 overflow-hidden xl:grid-cols-[430px_minmax(0,1fr)]">
      <nav
        className="itdb-hidden-scrollbar max-h-64 min-h-0 space-y-2 overflow-y-auto rounded-lg border border-[var(--itdb-border)] bg-white/[0.035] p-2 xl:max-h-none"
...
export function SettingsDetailPanel({
  ...
  return (
    <aside className="flex min-h-0 flex-col overflow-hidden rounded-lg border border-[var(--itdb-border)] bg-white/[0.035] p-4">
      {header}
      <div className="itdb-hidden-scrollbar min-h-0 flex-1 overflow-y-auto pt-1 pr-2">
        {children}
      </div>
      {actions ? (
        <div className="mt-5 flex shrink-0 flex-wrap justify-end gap-2 border-t border-[var(--itdb-border)] pt-4">
          {actions}
        </div>
      ) : null}
    </aside>
  ...
  return (
    <div className="mb-4 flex items-center gap-3">
      ...
      <div className="min-w-0">
        <div className="truncate text-sm font-semibold text-[var(--itdb-text)]">{title}</div>
        <div
          className="mt-0.5 truncate text-xs"
          style={{ color: active ? '#86efac' : 'var(--itdb-text-muted)' }}
        >
          {subtitle}
        </div>
      </div>
  ...
  return (
    <label
      ...
      style={{
        ...
        border: enabled ? '1px solid rgba(16,185,129,0.28)' : '1px solid var(--itdb-border)',
    ...
    >
      ...
      <span className="min-w-0">
        ...
        <span className="mt-0.5 block text-xs text-[var(--itdb-text-muted)]">
...
export function ConfigField({
  ...
  return (
    <label className="block space-y-1.5 text-xs" style={{ color: 'var(--itdb-text-muted)' }}>
      ...
      {field.type === 'select' ? (
        ...
        <textarea
          ...
          rows={3}
          className="itdb-hidden-scrollbar w-full resize-y rounded-lg px-3 py-2 text-sm outline-none disabled:opacity-60"
          style={{
        ...
        <span className="relative block">
          <input
            ...
            className={`w-full rounded-lg px-3 py-2 text-sm outline-none disabled:opacity-60 ${field.type === 'password' ? 'pr-10' : ''} ${field.type === 'number' ? 'itdb-number-input' : ''}`}
            style={{
              background: 'var(--itdb-control-bg)',
              border: '1px solid var(--itdb-border)',
              color: 'var(--itdb-text)',
          ...
          {field.type === 'password' ? (
            <AppTooltip label={passwordVisible ? '隐藏密码' : '显示密码'} placement="top">
              <button
                ...
                className="itdb-action-button absolute right-2 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-md disabled:opacity-40"
...
export function NumberControl({
  ...
  return (
    ...
    >
      <span className="flex items-center justify-between gap-3 text-sm font-semibold text-[var(--itdb-text)]">
        <span>{label}</span>
        <span className="shrink-0 rounded-md border border-blue-400/30 bg-blue-500/[0.08] px-2 py-0.5 text-xs text-[var(--itdb-accent-text)]">
          {normalized} {unit}
        </span>
      </span>
      ...
      <input
        ...
        className="itdb-flow-range mt-4 w-full disabled:opacity-60"
        style={{ '--itdb-range-progress': `${progress}%` } as React.CSSProperties}
      />
      <div className="mt-3 flex items-center gap-3">
        <input
          ...
          className="h-9 w-24 rounded-lg border border-[var(--itdb-border)] bg-[var(--itdb-control-bg)] px-3 text-sm text-[var(--itdb-text)] outline-none disabled:opacity-60"
        />
...
export function ActionButton({
  ...
  const palette = {
    primary: ['rgba(59,130,246,0.38)', 'var(--itdb-accent-text)', 'rgba(59,130,246,0.1)'],
    success: ['rgba(16,185,129,0.35)', '#34d399', 'rgba(16,185,129,0.08)'],
    danger: ['rgba(239,68,68,0.34)', '#f87171', 'rgba(239,68,68,0.08)'],
    muted: ['var(--itdb-border)', 'var(--itdb-text-muted)', 'rgba(255,255,255,0.035)'],
  }[tone];
  return (
    <button
      type="button"
      ...
      className={`itdb-action-button flex items-center gap-2 rounded-lg border px-3 py-2 text-sm disabled:cursor-not-allowed disabled:opacity-50 ${tone === 'danger' ? 'itdb-danger-button' : ''}`}
