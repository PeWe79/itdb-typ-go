...
func AllReportDefinitions() []ReportDefinition {
	return []ReportDefinition{
		...
		},
		{
			...
			Description: "各位置资产数量",
			Query: `SELECT COUNT(*) AS totalcount, locations.name AS Location, locations.floor AS Floor
		...
		},
		{
			...
			Query: `SELECT items.id AS ID, typedesc AS type, agents.title AS manufacturer, model, dnsname,
                    locations.name AS Location, locations.floor AS Floor,
                    (SELECT locareas.areaname FROM locareas WHERE locareas.id = items.locareaid) AS Area
                    FROM items
		...
		},
		{
			...
			Query: `SELECT items.id AS ID, items.ipv4, typedesc AS type, agents.title AS manufacturer, model, dnsname, label
                    FROM items, itemtypes, agents
		...
		{
			Name:        "allip6",
			Description: "已配置 IPv6 的资产列表",
			Query: `SELECT items.id AS ID, items.ipv6, typedesc AS type, agents.title AS manufacturer, model, dnsname, label
                    FROM items, itemtypes, agents
                    WHERE agents.id = manufacturerid AND itemtypes.id = items.itemtypeid AND ipv6 <> ''
                    ORDER BY ipv6`,
		},
		{
			Name:        "noinvoice",
			...
			Query: `SELECT items.id AS ID, typedesc AS type, agents.title AS manufacturer, model,
                    CASE WHEN purchasedate > 0 THEN strftime('%Y-%m-%d', purchasedate, 'unixepoch', 'localtime') ELSE '' END AS PurchaseDate
                    FROM items, itemtypes, agents
                    WHERE agents.id = manufacturerid
                      AND itemtypes.id = items.itemtypeid
		...
		},
		{
			...
			Query: `SELECT items.id AS ID, typedesc AS type, agents.title AS manufacturer, model,
                    ...
                    CASE WHEN CAST(purchprice AS NUMERIC) > 0 THEN CAST(ROUND(purchprice - (purchprice * 1.0 / 36) * CAST(((strftime('%s', 'now') - purchasedate)/(60*60*24*30.4)*(purchasedate AND 1)) AS INTEGER)) AS INTEGER) ELSE '' END AS CurrentValue
                    FROM items, itemtypes, agents
		...
		},
		{
			...
			Query: `SELECT items.id AS ID, typedesc AS type, agents.title AS manufacturer, model,
                    ...
                    purchprice AS PurchasePrice,
                    CAST(((strftime('%s', 'now') - purchasedate)/(60*60*24*30.4)*(purchasedate AND 1)) AS INTEGER) AS Months,
                    CASE WHEN CAST(purchprice AS NUMERIC) > 0 THEN CAST(ROUND(purchprice - (purchprice * 1.0 / 60) * CAST(((strftime('%s', 'now') - purchasedate)/(60*60*24*30.4)*(purchasedate AND 1)) AS INTEGER)) AS INTEGER) ELSE '' END AS CurrentValue
                    FROM items, itemtypes, agents
                    WHERE agents.id = manufacturerid AND itemtypes.id = items.itemtypeid`,
		},
	}
