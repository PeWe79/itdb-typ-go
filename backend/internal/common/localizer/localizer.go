...
var exactMessageCN = map[string]string{
	...
	"invalid macs":                                    "MAC 地址格式无效",
	"invalid ipv4":                                    "IPv4 地址格式无效",
	"invalid ipv6":                                    "IPv6 地址格式无效",
	"invalid remadmip":                                "远程管理 IP 格式无效",
	...
	"invalid ports":                                   "网络端口必须为 0-65535 之间的整数",
	"licenseqty cannot be negative":                   "授权数量不能为负数",
	"invoice file must be an image or pdf":            "发票类型的文件仅支持图片或 PDF 格式",
}

var (
	reAreaAssoc        = regexp.MustCompile(`^area has (\d+) associations$`)
	reLocationItems    = regexp.MustCompile(`^location is used by (\d+) items$`)
	reAreaRacks        = regexp.MustCompile(`^area is used by (\d+) racks$`)
	reLocationRacks    = regexp.MustCompile(`^location is used by (\d+) racks$`)
