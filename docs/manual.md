...
平台提供细粒度权限控制（内置三角色与自定义角色共 43 项权限）、本地与 AD/LDAP 双模式登录、找回密码邮件流程、六大模块的中文审计日志，以及手动/定时数据备份与旧项目数据库兼容导入。后端使用 SQLite（纯 Go 驱动，无 CGO 依赖）保存业务数据与系统配置，JWT 签名密钥与 LDAP 绑定密码等敏感信息以主密钥加密后落库。
...
- **用户认证与访问控制**：本地密码与 AD/LDAP 双模式登录、JWT 会话、找回密码邮件流程；内置 admin/operator/viewer 三角色，自定义角色按资产管理、资料管理、打印标签、统计报表、资产导航、审计日志、系统设置共 43 项权限划分，前端隐藏无权操作、后端逐接口校验。
...
- **统计与导航**：仪表盘资产概览、内置统计报表、资产导航树。
- **系统配置**：品牌标识、找回密码安全时效、定时备份参数、用户/用户组/角色、AD/LDAP 认证与邮件通知配置，并提供认证连通性与邮件发送测试。
...
公开访问仅限健康检查、登录、品牌信息与找回密码流程接口；其余接口均需 Bearer Token，并由后端权限中间件按 43 项权限逐接口校验。登录（含失败）、资产与配置变更、备份导入等关键操作均写入审计日志，删除被引用数据、创建重名等业务规则拒绝由前端明确提示、不产生审计噪音。
...
- **统计报表**：`reports.read`；**资产导航**：`browse.read`；**审计日志**：`audit.read`（查看）、`audit.manage`（导出，隐含 read）
...
- `POST /api/history/events` - 上报前端审计事件：前端执行数据导出、硬件维护日志导出、资料字典导入或标签打印后上报审计事件；事件类型白名单校验并按所属资源校验权限

### 5.2.13 数据库导入

- `POST /api/import/database` - 导入 SQLite 数据库：上传 .db 或 zip 替换当前数据库，压缩包内的上传文件恢复到上传目录，导入前自动备份；旧版平台数据库自动转换，仅迁移资产管理、资料管理与用户数据，系统配置、审计历史、标签预设保持当前默认
...
旧项目数据库（含旧版 PHP ITDB 导出的 .db）可通过「系统配置 → 数据备份 → 数据库导入」上传 .db 或包含上传文件的 .zip 自动完成迁移：系统会识别旧库结构并自动转换，仅迁移资产管理、资料管理与用户数据（旧「用户描述」对应现「显示名称」，邮箱留空后可手动补填），系统配置、审计历史与标签预设保持当前默认；状态类型自动从 1 重新编号并同步硬件状态引用，缺失的内置硬件类型自动补齐，英文内置名（文件类型/合同类型）自动转为中文。导入前会自动备份当前数据库。
